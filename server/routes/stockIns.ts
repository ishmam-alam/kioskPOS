...
import { PrismaClient, type StockIn } from "@prisma/client";
...
// Type definitions for stock in requests and responses
type CreateStockInRequest = Omit<StockIn, 'id' | 'received_at'>;
type UpdateStockInRequest = Partial<Pick<StockIn, 'added_qty' | 'supplier' | 'stock_adjusted'>>;
...
router.get(
  ...
  async (req: Request, res: Response<ApiResponse<StockIn[]>>) => {
    try {
...
// ----------------------------
router.get("/:id", attachUser, requireRoles([Role.ADMIN]), async (req: Request<{ id: string }>, res: Response<ApiResponse<StockIn>>) => {
  const { id } = req.params;
...
router.post(
  ...
  attachUser,
  requireRoles([Role.ADMIN]),
  async (req: Request<{}, {}, CreateStockInRequest>, res: Response<ApiResponse<StockIn>>) => {
...
router.patch(
  ...
  attachUser,
  requireRoles([Role.ADMIN]),
  async (req: Request<{ id: string }, {}, UpdateStockInRequest>, res: Response<ApiResponse<StockIn>>) => {
    const { id } = req.params;
    ...
    try {
      ...
      if (added_qty !== undefined) updateData.added_qty = added_qty;
