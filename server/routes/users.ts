...
// Type definitions for user responses
type PublicUserInfo = Pick<User, 'user_id' | 'name' | 'role'>;
type UserWithoutPin = Omit<User, 'pin'>;
type CreateUserRequest = Pick<User, 'user_id' | 'pin' | 'name' | 'role' | 'active'>;
...
router.get(
  ...
  async (req: Request, res: Response<ApiResponse<(User | PublicUserInfo | UserWithoutPin)[]>>) => {
    try {
      const users = await prisma.user.findMany({ orderBy: { created_at: "desc" } });
      const requestingRole = req.user?.role;

      const data = users
        .map((u): User | PublicUserInfo | UserWithoutPin | null => {
        ...
        .filter((user): user is User | PublicUserInfo | UserWithoutPin => user !== null);
...
// GET user by user_id
// ----------------------------
router.get("/:user_id", attachUser, async (req: Request<{ user_id: string }>, res: Response<ApiResponse<User | PublicUserInfo | UserWithoutPin>>) => {
  const { user_id } = req.params;
  const requestingUser = req.user;

  try {
    const user = await prisma.user.findUnique({ where: { user_id: Number(user_id) } });
    if (!user) return res.status(404).json({ success: false, error: `User with user_id "${user_id}" not found` });

    // Self access
    if (requestingUser?.userId === Number(user_id)) return res.json({ success: true, data: user });
    ...
    let masked: User | PublicUserInfo | UserWithoutPin = user;
...
router.post(
  ...
  async (req: Request<{}, {}, CreateUserRequest>, res: Response<ApiResponse<User>>) => {
    try {
...
router.patch(
  "/:user_id",
  attachUser,
  validateParams(idParamSchema, "user_id"),
  validateRequest(updateUserSchema),
  async (req: Request<{ user_id: string }, {}, UpdateUserRequest>, res: Response<ApiResponse<User>>) => {
    const user_id = (req as any).validatedParams.user_id;
    const requestingUser = req.user;
    const { pin, name, role, active } = req.body;

    if (!requestingUser) return res.status(401).json({ success: false, error: "Authentication required" });
    ...
    let allowedFields: UpdateUserRequest = {};
    
    if (isAdmin) {
      if (pin !== undefined) allowedFields.pin = pin;
...
// DELETE user
// ----------------------------
router.delete("/:user_id", attachUser, requireRoles([Role.ADMIN, Role.OWNER]), validateParams(idParamSchema, "user_id"), async (req: Request<{ user_id: string }>, res: Response<ApiResponse<null>>) => {
  const user_id = (req as any).validatedParams.user_id;
  try {
...
// LOGIN endpoint
// ----------------------------
router.post("/login", validateRequest(loginSchema), async (req: Request<{}, {}, { user_id: number; pin: number }>, res: Response<ApiResponse<{ token: string; user: User }>>) => {
...
// ----------------------------
router.get("/me", attachUser, async (req: Request, res: Response<ApiResponse<Omit<User, 'pin'>>>) => {
  if (!req.user) {
...
// WELCOME endpoint
// ----------------------------
router.get("/welcome", async (req: Request, res: Response<ApiResponse<{ message: string }>>) => {
