...
router.post("/", attachUser, requireRoles([Role.ADMIN]), async (req, res) => {
    ...
    try {
        ...
        if (product_id !== undefined)
            createData.product_id = product_id;
        if (user_id !== undefined)
            createData.user_id = user_id;
        if (change_qty !== undefined)
            createData.change_qty = change_qty;
        if (resulting_qty !== undefined)
            createData.resulting_qty = resulting_qty;
        if (movement_type !== undefined)
            createData.movement_type = movement_type;
        if (reference_table !== undefined)
            createData.reference_table = reference_table;
        if (reference_id !== undefined)
            createData.reference_id = reference_id;
        if (note !== undefined)
...
router.patch("/:id", attachUser, requireRoles([Role.ADMIN]), async (req, res) => {
    ...
    try {
        ...
        const updateData = {};
        if (change_qty !== undefined)
            updateData.change_qty = change_qty;
        if (resulting_qty !== undefined)
            updateData.resulting_qty = resulting_qty;
        if (movement_type !== undefined)
            updateData.movement_type = movement_type;
        if (reference_table !== undefined)
            updateData.reference_table = reference_table;
        if (reference_id !== undefined)
